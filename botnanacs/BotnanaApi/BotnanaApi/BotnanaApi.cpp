...
extern "C"
{
	...
	__declspec(dllexport) void botnana_subscribe_ec_slave(
		struct Botnana *botnana,
		uint32_t alias,
		uint32_t position)
	{
		subscribe_ec_slave(botnana, alias, position);
	}

	// JSON-API: ec_slave.unsubscribe
	__declspec(dllexport) void botnana_unsubscribe_ec_slave(
		struct Botnana *botnana,
		uint32_t alias,
		uint32_t position)
	{
		unsubscribe_ec_slave(botnana, alias, position);
	}
}
