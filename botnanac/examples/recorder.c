    ...
    while(1)
    {
        ...
        if (strlen(buffer) > 0)
        {
            if (strstr(buffer,"trigger") != 0)
            {
                ...
                fprintf(desc->fp,"time,real,target\n");
                desc->trigger = 1;
...
int main()
{
    ...
    // new program
    desc.pm = program_new("recorder");
    program_line(desc.pm, "0 begin 1 + dup ");
    program_line(desc.pm, "10000 <= while");
    program_line(desc.pm, ".\" test-recorder|\" mtime . 44 emit 1 1 real-p@ . 44 emit 1 1 target-p@ . cr ");
    program_line(desc.pm, "pause");
    program_line(desc.pm, "repeat drop");
    program_line(desc.pm, "100 ms");
