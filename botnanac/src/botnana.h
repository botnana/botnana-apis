...
#include <stdint.h>
        ...
        // Get modbus i16 value from input or holding register at `addr`.
        int16_t botnana_mb_i16(struct Botnana *desc, uint addr);

        // Get modbus u16 value from input or holding register at `addr`.
        uint16_t botnana_mb_u16(struct Botnana *desc, uint addr);

        // Get modbus i32 value from input or holding registers at `addr` and `addr+1`.
        int32_t botnana_mb_i32(struct Botnana *desc, uint addr);

        // Get modbus u32 value from input or holding registers at `addr` and `addr+1`.
        uint32_t botnana_mb_u32(struct Botnana *desc, uint addr);

        // Publish modbus holding registers
        void botnana_mb_publish(struct Botnana *desc);

        // Set modbus coil at `addr`.
        void botnana_mb_set_bit(struct Botnana *desc, uint addr, int value);

        // Set modbus i16 value at holding register `addr`.
        void botnana_mb_set_i16(struct Botnana *desc, uint addr, int16_t value);

        // Set modbus u16 value at holding register `addr`.
        void botnana_mb_set_u16(struct Botnana *desc, uint addr, uint16_t value);

        // Set modbus i32 value at holding registers `addr` and `addr+1`.
        void botnana_mb_set_i32(struct Botnana *desc, uint addr, int32_t value);

        // Set modbus u32 value at holding registers `addr` and `addr+1`.
        void botnana_mb_set_u32(struct Botnana *desc, uint addr, uint32_t value);
